...
(function () {
  ...
  /* ⚠️ DERIVE THE API ONLY WHEN THIS FILE IS SERVED BY THE WORKER.
   *
   * The first version did `src.replace(/\/embed\.js$/, "")` unconditionally,
   * which is right when the tag points at the worker and WRONG the moment a site
   * self-hosts the file — crispvideo.app serves /founding.js and /notify.js from
   * its own root, so a copied /embed.js would derive "https://crispvideo.app" and
   * POST the questions to a page instead of the assistant.
   *
   * It would not throw. It would 404 quietly and the widget would say it could
   * not reach the server, on a site where everything else worked.
   */
  var WORKER = "https://kerr-lead-agent.kerrco.workers.dev";
  var API = WORKER;
  try {
    var src = (document.currentScript && document.currentScript.src) || "";
    if (src && src.indexOf("kerr-lead-agent") > -1) {
      API = src.replace(/\/embed\.js.*$/, "") || WORKER;
    }
  } catch (e) { API = WORKER; }
